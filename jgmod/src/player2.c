...
void lock_jgmod_player2(void)
{
    ...
    LOCK_FUNCTION(calc_pan);
    LOCK_FUNCTION(note2period);
    LOCK_FUNCTION(get_jgmod_sample_no);
    LOCK_FUNCTION(period2pitch);
    LOCK_FUNCTION(do_position_jump);
    LOCK_FUNCTION(do_pattern_break);
    LOCK_FUNCTION(do_pro_tempo_bpm);
    LOCK_FUNCTION(do_pattern_loop);
    LOCK_FUNCTION(do_vibrato);
    LOCK_FUNCTION(do_tremolo);
    LOCK_FUNCTION(do_slide2period);
    LOCK_FUNCTION(do_arpeggio);
    LOCK_FUNCTION(do_delay_sample);

    LOCK_FUNCTION(parse_pro_pitch_slide_up);
    LOCK_FUNCTION(parse_pro_pitch_slide_down);
    LOCK_FUNCTION(parse_pro_volume_slide);
    LOCK_FUNCTION(parse_slide2period);
    LOCK_FUNCTION(parse_vibrato);
    LOCK_FUNCTION(parse_tremolo);
    LOCK_FUNCTION(parse_pro_arpeggio);
    LOCK_FUNCTION(parse_old_note);

#ifdef __ALLEGRO_DJGPP__
    _go32_dpmi_lock_data((void *)noteperiod, sizeof(noteperiod[0]) * 12);
    _go32_dpmi_lock_data((void *)vib_table, sizeof(vib_table[0]) * 24);
    _go32_dpmi_lock_data((void *)voice_table, sizeof(voice_table[0]) * MAX_ALLEG_VOICE);
    _go32_dpmi_lock_data((void *)logtab, sizeof(logtab[0]) * 104);
    _go32_dpmi_lock_data((void *)mod_finetune, sizeof(mod_finetune[0]) * 16);
    _go32_dpmi_lock_data((void *)lintab, sizeof(lintab[0]) * 768);
#endif
}
