...
static BITMAP *generate_bitmap(AL_CONST char *template[], int w, int h)
{
...
static void win95_text_bevel(BITMAP *bmp, int x, int y, int w, int h, int state)
{
    hline(bmp, x, y, x+w-2, nshadow);
    vline(bmp, x, y, y+h-2, nshadow);
    hline(bmp, x+1,y+1,x+w-3, black);
...
static void win95_menu_box(BITMAP *bmp, int x, int y, int w, int h, int state, int border)
{
...
int d_awin95_list_proc(int msg, DIALOG *d, int c)
{
	...
	win95_draw_scrollable_frame(d, listsize, d->d2, height, fg_color);
...
int d_awin95_textbox_proc(int msg, DIALOG *d, int c)
{
	...
	win95_draw_scrollable_frame(d, d->d1, d->d2, height, fg_color);
	return D_O_K;
    }
