...
//Code from SVAsync lib.
/*****Clear 8250 Status and data registers */
//setting up FIFO if possible
  { outportb(port->SCR,0x55);
    if(inportb(port->SCR)==0x55)
     { outportb(port->FCR,0x0f);
       if((inportb(port->IIR)&0xc0)==0xc0)
         { outportb(port->FCR,0x01);
           outportb(port->FCR,0x02);
           outportb(port->FCR,1|2|4|64); // 8 byte trigger level on receive.
           outportb(port->FCR,inportb(port->FCR)&0xfe);
         }
     }
  }
  { unsigned char temp;
...
{ int int_id;
  ...
  while (1) //loop while !end of all interrupts
   { int_id=inportb(port->IIR); //get interrupt id
     if ((int_id&0x01)==1) //no interrupts
      { DISABLE();
        outportb(0x20,0x20);
